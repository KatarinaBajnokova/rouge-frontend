...
import { u as E, h as F, C as q } from './index-Cd1dwsJR.js';
import {
  G as x,
  L as v,
  e as a,
...
function J() {
  const [k, i] = d.useState([]),
    [y, r] = d.useState(0),
    [C, p] = d.useState(!0),
    { userId: h, loading: u } = E(),
    o = N(),
    c = 'basket',
    l = () => {
  ...
  d.useEffect(() => {
    if (localStorage.getItem('resetBasket') === 'true')
      ...
      const n = JSON.parse(localStorage.getItem(c) || 'null');
      n && (i(n), r(n.reduce((j, b) => j + b.price * b.quantity, 0)));
    }
    l();
    const t = n => {
      n.key === c && (n.newValue ? l() : (i([]), r(0)));
    };
    return (
  ...
  const w = async s => {
      await fetch(`/api/basket/${s}`, { method: 'DELETE' }), l();
    },
    f = async (s, t) => {
      t < 1 ||
        t > 10 ||
    ...
    m = 4.99,
    I = (y + m).toFixed(2).replace('.', ',');
  if (C)
    return e.jsxs(x, {
      position: 'center',
      mt: 'lg',
      children: [e.jsx(v, {}), e.jsx(a, { children: 'Loading basket…' })],
    });
  const S = async () => {
    if (!u) {
      if ((console.log('Checking checkout logic. Firebase UID:', h), !h)) {
        console.log('❌ No firebase user detected, navigate to personal info.'),
          o('/checkout/personal-info');
        return;
      }
      try {
        (await fetch(`/api/users/by-firebase-uid?uid=${h}`)).ok
          ? (console.log('✅ Backend user exists, navigating to friend info'),
            o('/checkout/friend-info'))
          : (console.log(
              '❌ Backend user not found, navigating to personal info'
            ),
            o('/checkout/personal-info'));
      } catch (s) {
        console.error('❌ Error verifying user in backend:', s),
          o('/checkout/personal-info');
      }
    }
  };
  return e.jsxs('div', {
    ...
    children: [
      e.jsx(F, { text: 'Shopping Basket' }),
      k.length === 0
        ? e.jsxs('div', {
            className: 'empty-basket',
            children: [
              e.jsx('img', { src: V, alt: 'Empty basket' }),
              e.jsx('p', { children: 'Your basket is empty.' }),
            ],
          })
        : e.jsxs(e.Fragment, {
            children: [
              e.jsx(a, { fw: 700, mt: 'md', mb: 'xs', children: 'Your items' }),
              e.jsx(T, {
                spacing: 'sm',
                children: k.map(s =>
                  e.jsxs(
                    'div',
                    {
                      ...
                      children: [
                        e.jsx(z, { src: s.image_url, radius: 'md' }),
                        e.jsxs('div', {
                  ...
                  e.jsxs('div', {
                    ...
                    children: [
                      ...
                      e.jsxs(a, {
                        ...
                        mt: 'xs',
                        c: 'dimmed',
                        children: ['€', m.toFixed(2).replace('.', ',')],
                      }),
                      e.jsx(a, { mt: 'xs', fw: 700, children: 'Total:' }),
                      e.jsxs(P, { order: 2, mt: -6, children: ['€', I] }),
                    ],
                  }),
                  e.jsx(q, {
                    disabled: u,
